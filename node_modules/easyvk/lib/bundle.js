...
var EventEmitter$1 = _interopDefault(require('fast-event-emitter'));
...
class Debugger extends EventEmitter$1 {
  constructor (...props) {
    super(...props);
...
var configuration = {
  ...
  WINDOWS_CLIENT_ID: '3697615',
  WINDOWS_CLIENT_SECRET: 'AlVXZFMUqyrnABp8ncuU',

  ANDROID_CLIENT_ID: '2274003',
  ANDROID_CLIENT_SECRET: 'hHbZxrka2uZ6jB1inYsH',

  IOS_CLIENT_ID: '3140623',
  IOS_CLIENT_SECRET: 'VeWdmVclDCtn6ihuP1nt',
  ...
  DEFAULT_UTILS: {},
  DEFAULT_USER_AGENT: 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/67.0.3396.99 Safari/537.36'
};
...
class LongPollConnection extends EventEmitter$1 {
  constructor (lpSettings, vk) {
    ...
    async function init () {
      ...
      self.lpConnection = fetch$1(server, params).then(async (res) => {
        res = await res.json();
...
class CallbackAPI extends EventEmitter$1 {
  constructor (vk) {
    super();
    let self = this;
    self._vk = vk;
...
class StreamingAPIConnection extends EventEmitter$1 {
...
class LongPollConnection$1 extends EventEmitter$1 {
  constructor (lpSettings, vk) {
    super();
    let self = this;
  ...
  constructor (params, debuggerRun) {
    ...
    this.debug = (t, d) => {
      if (this.params.debug) {
        this.params.debug.emit(Debugger._toRaw(t), d);
      }
    };
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!session.access_token) { // If session file contents access_token, try auth with it
        if (params.access_token) {
          ...
          self.debug(Debugger.EVENT_REQUEST_TYPE, {
            url: url,
            query: getData,
            method: 'GET'
          });
      ...
      function prepareResponse (err, res) {
        ...
        self.debug(Debugger.EVENT_RESPONSE_TYPE, { body: vkr });
